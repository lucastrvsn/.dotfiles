# lucastrvsn's zsh configuration.
...
zstyle ':completion:*:*:kill:*:processs' list-colors '=(#b) #([0-9]#) ([0-9a-z-]#)*=01;36=0=01'
...
zstyle ':completion:*:(ssh|scp|rsync):*:hosts-ipaddr' ignored-patterns '^(<->.<->.<->.<->|(|::)([[:xdigit:].]##:(#c,2))##(|%*))' '127.0.0.<->' '255.255.255.255' '::1' 'fe80::*'e
...
# cross-plataform ls flags (unix and macos)
ls --version &>/dev/null
if [ $? -eq 0 ]; then
